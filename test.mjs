...
r = parse(await I.createSkill('flat-skill', '---\nname: flat-skill\ndescription: dup over single-file.\n---\nbody\n', SKILLS))
check('create over single-file layout refused', !r.success && /single-file skill/.test(r.error), JSON.stringify(r))
check('no dir-layout orphan created', !existsSync(path.join(SKILLS, 'flat-skill', 'SKILL.md')))
